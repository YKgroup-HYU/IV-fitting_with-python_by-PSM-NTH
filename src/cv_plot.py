...
def cvplot(A,B):
    ...
    for i in csv_tqdm:
        ...
        Data = pd.read_csv(".\dat\%s"%filename+'.csv',names=['Value','Vsweep','Cdata','four','five','six','7','8','9','10','11','12','13'])
        ...
        find_columns_v = find_row['Vsweep']
        find_columns_c = find_row['Cdata']
        ...
        plt.title('C-V graph_linear',fontsize = 30)
        plt.xlabel('Voltage [V]',fontsize = 30,labelpad=10)
        plt.ylabel('Capicitance [F]',fontsize = 30,labelpad=10)
        ...
        plt.yscale('log')
        plt.title('C-V graph_abs',fontsize = 30)
        plt.xlabel('Voltage [V]',fontsize = 30,labelpad=10)
        plt.xticks(fontsize = 25)
        plt.yticks(fontsize=30)
        plt.ylabel('Capicitance [F]',fontsize = 30,labelpad=10)
